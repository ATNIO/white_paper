...
### 支付通道
很多AI服务由于高频调用，会带来很大的通信开销。如果在底层设计上如果对每一个服务调用都进行计费和结算，需要付出巨大的性能代价和相对高额的手续费。因此ATN采用了一种支持高频调用的微支付技术作为服务接口设计中的一个核心组件，即**支付通道技术**。后面的时序图将会介绍如何使用支付通道。我们相信这会对新用户在实际使用过程中带来很大的助益。
